		...
		foreach($book->c AS $chapter){
			...
			foreach($chapter->p AS $paragraph){
				...
				foreach($paragraph->v AS $verse){
					...
					$string .=  '<strong>'.$book->h . ' ' . $chapter['id'].':' . $verse['id'] . '</strong>';
					
					$string .= $verse->children().'<br>';
...
Route::get('/test-react/{test}', 'TestReactController@test');
...
Route::get('search', ['as' => 'search_path','uses' => 'SearchesController@index']);
